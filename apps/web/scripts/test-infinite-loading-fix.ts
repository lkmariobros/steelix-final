...
function testDateStability() {
	...
	for (const range of timeRanges) {
		...
		const startSame =
			Math.abs(call1.startDate.getTime() - call2.startDate.getTime()) < 10;
		const endSame =
			Math.abs(call1.endDate.getTime() - call2.endDate.getTime()) < 10;
		...
		if (startSame && endSame) {
			...
			console.log(
				`    Difference: ${Math.abs(call1.startDate.getTime() - call2.startDate.getTime())}ms`,
			);
		}
