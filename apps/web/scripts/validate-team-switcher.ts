...
class TeamSwitcherValidator {
	...
	validateTeamSwitcherComponent() {
		...
		if (!this.fileExists(componentPath)) {
			this.addResult(
				false,
				"Team switcher component file missing",
				componentPath,
			);
			return;
		}
		...
		const hasTrpcRoleCheck = /trpc\.admin\.checkAdminRole\.useQuery/.test(
			componentContent,
		);
		this.addResult(
			hasTrpcRoleCheck,
		...
		const hasAriaLabels = /aria-label|aria-expanded|aria-haspopup/.test(
			componentContent,
		);
		this.addResult(
			hasAriaLabels,
	...
	validateServerSideChanges() {
		...
		if (this.fileExists(`../server/${trpcPath}`)) {
			...
			this.addResult(
				...
				!hasStrictAdmin
					? "Missing strict admin check in adminProcedure"
					: undefined,
			);
		}
		...
		if (this.fileExists(`../server/${adminRouterPath}`)) {
			...
			this.addResult(
				...
				!hasAdminOnlyCheck
					? "Missing admin-only check in checkAdminRole"
					: undefined,
			);
		}
	}
	...
	validateSupportingFiles() {
		...
		const hasLoadingSpinner = this.fileExists(
		...
		const hasErrorBoundary = this.fileExists(
		...
		const hasAccessDenied = this.fileExists(
		...
		const hasKeyboardHook = this.fileExists(
		...
		// Check for test file
		const hasTests = this.fileExists(
			"src/components/__tests__/team-switcher.test.tsx",
		);
		this.addResult(
			hasTests,
			"Test file exists",
	...
	validateAppSidebarCleanup() {
		...
		const hasUnusedAuthImport = /import.*authClient.*from.*auth-client/.test(
			sidebarContent,
		);
		this.addResult(
			!hasUnusedAuthImport,
	...
	validateAdminSettingsPage() {
		...
		const hasBypassComments =
			/TEMPORARILY ALLOW ACCESS|TODO.*Re-enable role check/.test(
				settingsContent,
			);
		this.addResult(
			!hasBypassComments,
		...
		const hasTrpcImport = /import.*trpc.*from.*utils\/trpc/.test(
			settingsContent,
		);
		this.addResult(
			hasTrpcImport,
