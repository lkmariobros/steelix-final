...
export type ApprovalQueueTransaction = {
	id: string;
	caseNo?: string | null;
...
type ApprovalCategory = "new-project" | "subsale" | "rental";
...
function getApprovalCategory(tx: ApprovalQueueTransaction): ApprovalCategory {
	...
	if (category === "new-project") {
		return (
			<div className="grid gap-x-6 gap-y-1 text-sm sm:grid-cols-2 lg:grid-cols-4">
				...
				<DetailField
					label="Submit Date"
					value={formatDate(tx.submittedAt)}
				/>
			</div>
		);
	}
	...
	if (category === "subsale") {
		return (
			<div className="grid gap-x-6 gap-y-1 text-sm sm:grid-cols-2 lg:grid-cols-3">
				<DetailField
					label="Offer Date"
					value={formatDate(tx.transactionDate)}
				/>
				<DetailField
					label="Address"
					value={prop?.address?.trim() || "—"}
				/>
				<DetailField label="Nett Price" value={formatRm(nett)} />
				<DetailField
					label="Commission Amount"
...
export function ApprovalQueueItem({
	...
	onApprove: (tx: ApprovalQueueTransaction) => void;
	onReject: (tx: ApprovalQueueTransaction) => void;
}) {
