...
export async function updateLeadAdmin(
	leadId: string,
	data: Partial<InsertProspect> & { tagIds?: string[] },
) {
 ...
 */
export async function assignLeadAdmin(leadId: string, agentId: string | null) {
	const [updated] = await db
		.update(prospects)
...
export async function createLeadAdmin(
	data: InsertProspect & { agentId?: string | null; tagIds?: string[] },
) {
