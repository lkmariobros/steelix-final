...
export type LeadTaskReportRow = LeadTask & {
	agentId: string | null;
	agentName: string | null;
};

export type LeadTaskReportSummary = {
	open: number;
	overdue: number;
	completed: number;
	dueToday: number;
...
/**
 * Admin report across all agent-created lead tasks.
 */
export async function getLeadTasksReport(opts: {
	agentId?: string;
	status?: "open" | "completed" | "overdue";
	limit?: number;
	offset?: number;
}): Promise<{
	tasks: LeadTaskReportRow[];
	summary: LeadTaskReportSummary;
	total: number;
}> {
	...
	if (opts.agentId) {
		filters.push(`p.agent_id = $${idx++}`);
		values.push(opts.agentId);
	}

	if (opts.status === "open") {
		filters.push("t.completed_at IS NULL");
	} else if (opts.status === "completed") {
		filters.push("t.completed_at IS NOT NULL");
	} else if (opts.status === "overdue") {
		filters.push("t.completed_at IS NULL AND t.due_date < NOW()");
	}
	...
	const summaryRes = await pool.query(
      ...
      COUNT(*) FILTER (
        WHERE t.completed_at IS NULL AND t.due_date < NOW()
      )::int AS overdue,
      COUNT(*) FILTER (WHERE t.completed_at IS NOT NULL)::int AS completed,
      COUNT(*) FILTER (
        WHERE t.completed_at IS NULL AND t.due_date::date = CURRENT_DATE
      )::int AS due_today
     FROM public.lead_tasks t
     INNER JOIN public.prospects p ON p.id = t.prospect_id
     WHERE ${where}`,
    ...
    FROM  public.lead_tasks t
    INNER JOIN public.prospects p ON p.id = t.prospect_id
    LEFT JOIN public."user" ag ON ag.id = p.agent_id
    LEFT JOIN public."user" au ON au.id = t.assigned_to
    LEFT JOIN public."user" cu ON cu.id = t.created_by
    WHERE ${where}
    ORDER BY t.due_date ASC
...
export async function getProspectAgentId(
	prospectId: string,
): Promise<string | null> {
	const res = await pool.query(
		`SELECT agent_id FROM public.prospects WHERE id = $1 LIMIT 1`,
		[prospectId],
	);
	return (res.rows[0]?.agent_id as string | null) ?? null;
}

export async function getTaskProspectAgentId(
	taskId: string,
): Promise<{ prospectId: string; agentId: string | null } | null> {
	const res = await pool.query(
		`SELECT t.prospect_id, p.agent_id
     FROM public.lead_tasks t
     INNER JOIN public.prospects p ON p.id = t.prospect_id
     WHERE t.id = $1
     LIMIT 1`,
