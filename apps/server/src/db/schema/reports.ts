...
export const performanceMetrics = pgTable(
	"performance_metrics",
	{
		...
		completedTransactions: integer("completed_transactions")
			.default(0)
			.notNull(),
		...
		totalCommission: decimal("total_commission", { precision: 12, scale: 2 })
			.default("0")
			.notNull(),
		averageCommission: decimal("average_commission", {
			precision: 10,
			scale: 2,
		})
			.default("0")
			.notNull(),
		totalSalesVolume: decimal("total_sales_volume", { precision: 15, scale: 2 })
			.default("0")
			.notNull(),
		...
		conversionRate: decimal("conversion_rate", { precision: 5, scale: 2 })
			.default("0")
			.notNull(),
		averageDaysToClose: decimal("average_days_to_close", {
			precision: 5,
			scale: 1,
		})
			.default("0")
	...
	(table) => ({
		...
		periodIdx: index("idx_performance_metrics_period").on(
			table.periodType,
			table.periodStart,
			table.periodEnd,
		),
		calculatedAtIdx: index("idx_performance_metrics_calculated_at").on(
			table.calculatedAt,
		),
	}),
);
...
export const analyticsDashboards = pgTable(
	...
	(table) => ({
		...
		createdByIdx: index("idx_analytics_dashboards_created_by").on(
			table.createdBy,
		),
