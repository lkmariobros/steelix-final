...
export const commissionApprovals = pgTable(
	...
	(table) => ({
		...
		reviewedByIdx: index("idx_commission_approvals_reviewed_by").on(
			table.reviewedBy,
		),
		submittedAtIdx: index("idx_commission_approvals_submitted_at").on(
			table.submittedAt,
		),
	}),
);
...
export const approvalWorkflowHistory = pgTable(
	...
	(table) => ({
		approvalIdIdx: index("idx_approval_workflow_history_approval_id").on(
			table.approvalId,
		),
		actionByIdx: index("idx_approval_workflow_history_action_by").on(
			table.actionBy,
		),
		timestampIdx: index("idx_approval_workflow_history_timestamp").on(
			table.timestamp,
		),
	}),
);
...
export type NewCommissionApproval = z.infer<
	typeof insertCommissionApprovalSchema
>;
export type UpdateApprovalStatus = z.infer<typeof updateApprovalStatusSchema>;
export type ApprovalWorkflowHistory = z.infer<
	typeof approvalWorkflowHistorySchema
>;
