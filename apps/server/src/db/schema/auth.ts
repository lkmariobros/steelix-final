...
export const tierCommissionConfig = pgTable(
	"tier_commission_config",
	{
		id: uuid("id").primaryKey().defaultRandom(),
	...
	(table) => ({
		...
		isActiveIdx: index("idx_tier_commission_config_is_active").on(
			table.isActive,
		),
	}),
);
...
export const leadershipBonusPayments = pgTable(
	"leadership_bonus_payments",
	{
		id: uuid("id").primaryKey().defaultRandom(),
		transactionId: uuid("transaction_id").notNull(), // References transactions table
		...
		// Commission details
		originalCommissionAmount: decimal("original_commission_amount", {
			precision: 15,
			scale: 2,
		}).notNull(),
		companyShareAmount: decimal("company_share_amount", {
			precision: 15,
			scale: 2,
		}).notNull(),
		leadershipBonusRate: integer("leadership_bonus_rate").notNull(), // Percentage applied
		leadershipBonusAmount: decimal("leadership_bonus_amount", {
			precision: 15,
			scale: 2,
		}).notNull(),
		// Status
	...
	(table) => ({
		transactionIdIdx: index("idx_leadership_bonus_transaction_id").on(
			table.transactionId,
		),
		downlineAgentIdIdx: index("idx_leadership_bonus_downline_agent_id").on(
			table.downlineAgentId,
		),
		uplineAgentIdIdx: index("idx_leadership_bonus_upline_agent_id").on(
			table.uplineAgentId,
		),
...
export const tierConfigChangeLog = pgTable(
	"tier_config_change_log",
	{
		id: uuid("id").primaryKey().defaultRandom(),
	...
	(table) => ({
		configIdIdx: index("idx_tier_config_change_log_config_id").on(
			table.configId,
		),
		tierIdx: index("idx_tier_config_change_log_tier").on(table.tier),
		timestampIdx: index("idx_tier_config_change_log_timestamp").on(
			table.timestamp,
		),
	}),
);
...
export type LeadershipBonusPayment = z.infer<
...
export const agentActivities = pgTable(
	...
	(table) => ({
		...
		activityTypeIdx: index("idx_agent_activities_activity_type").on(
			table.activityType,
		),
